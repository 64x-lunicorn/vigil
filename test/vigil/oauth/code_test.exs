defmodule Vigil.OAuth.CodeTest do
  ...
  defp ctx! do
    {:ok, %{client_id: client_id}} =
      Flow.register(%{"redirect_uris" => [@redirect_uri]}, @now)
  ...
  describe "issue/2" do
    test "writes the client, redirect URI, challenge, audience and scope it was consented for" do
      ctx = ctx!()
      code = Code.issue(ctx, @now)
    ...
    # replay revokes.
    test "every code opens a grant of its own" do
      {:ok, first} = Store.take_code(Code.issue(ctx!(), @now))
      {:ok, second} = Store.take_code(Code.issue(ctx!(), @now))

      assert is_binary(first.grant_id)
      assert first.grant_id != second.grant_id
    end

    test "a code lives one minute" do
      {:ok, record} = Store.take_code(Code.issue(ctx!(), @now))
  ...
  describe "redeem/3" do
    test "a code presented as it was issued comes back with its record" do
      ctx = ctx!()
      code = Code.issue(ctx, @now)

      assert {:ok, record} = Code.redeem(code, request(ctx), @now)
      assert Code.audience_of(record) == OAuth.resource()
    end

    test "a code the store has never seen" do
      assert {:error, :unknown} = Code.redeem("nope", request(ctx!()), @now)
    end

    test "a code past its minute" do
      ctx = ctx!()
      code = Code.issue(ctx, @now)

      assert {:error, :expired} = Code.redeem(code, request(ctx), @now + 60)
    end

    test "a code redeemed by a client it was not issued to" do
      ctx = ctx!()
      code = Code.issue(ctx, @now)

      assert {:error, :wrong_client} =
               Code.redeem(code, request(ctx, %{"client_id" => "someone-else"}), @now)
    end

    test "a code redeemed against a redirect URI it was not checked against" do
      ctx = ctx!()
      code = Code.issue(ctx, @now)
    ...
    test "a verifier that does not hash to the stored challenge, and a missing one" do
      ctx = ctx!()
      ...
      assert {:error, :bad_pkce} =
               Code.redeem(Code.issue(ctx, @now), Map.delete(request(ctx), "code_verifier"), @now)
    end
    ...
    test "a failed redemption still consumes the code" do
      ctx = ctx!()
      code = Code.issue(ctx, @now)
      ...
      assert {:error, :unknown} = Code.redeem(code, request(ctx), @now)
    end
  end
end
