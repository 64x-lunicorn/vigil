defmodule Vigil.MCP.Envelope.DecisionTest do
  ...
  describe "for_tool/4 — the current tool" do
  ...
  describe "for_tool/4 — first call in the session (prev_state nil)" do
    test "no near event: header only" do
      {result, _state} = Decision.for_tool("search", nil, @now, snapshot())
    ...
    test "an active near event is summarized in English with its note title, ends_in 'left'" do
      ...
      {result, _state} = Decision.for_tool("search", nil, @now, snapshot(near: near))
    ...
    test "newly active event names itself with its title, not the filename stem" do
      ...
      {result, _state} =
        Decision.for_tool(
          ...
          prev_state,
          @now,
          snapshot(active_ids: active_ids, titles: titles)
        )
    ...
    test "newly inactive event reports 'now finished'" do
      ...
      {result, _state} =
        Decision.for_tool(
          ...
          prev_state,
          @now,
          snapshot(active_ids: MapSet.new(), titles: titles)
        )
